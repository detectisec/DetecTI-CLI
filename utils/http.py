...
class AsyncHTTPClient:
    ...
    async def request(
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                if attempt >= self.max_retries:
                    logger.warning(f"HTTP request to {url} failed after {self.max_retries} attempts: {exc}")
                    raise
                sleep_time = self.backoff_factor * (2 ** (attempt - 1)) + random.uniform(0.1, 0.5)
                await asyncio.sleep(sleep_time)

        raise httpx.RequestError(f"Failed to complete request to {url} after {self.max_retries} attempts")
    ...
    async def get_json(
        ...
        try:
            resp = await self.get(url=url, headers=headers, params=params, timeout=timeout, raise_for_status=False)
