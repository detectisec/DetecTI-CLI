...
dom_nodes, dom_edges, _, subdomain_to_ips, domain_to_ips, explicit_targets = builder._build_domain_nodes(conn, active_targets=[str(sub_id)])
for n in dom_nodes:
    if str(sub_id) in n["data"]["id"]:
        print(f"Found node: {n['data']['id']}")
print(f"Total domain nodes: {len(dom_nodes)}")
