...
def patch_graph_builder():
    ...
    # 1. Fix _build_domain_nodes to only spawn explicit targets and append explore_leads
    replacement_domain = """
...
"""
    ...
    content = re.sub(pattern, replacement_domain + "\n\n        return nodes, edges, root_target_node, subdomain_to_ips, domain_to_ips, explicit_targets", content, flags=re.DOTALL)

    # 2. Fix IP connection logic so that IPs ONLY spawn if they are explicit targets (or discovery).
    # And they connect to root if they have no visible FQDN parent.
    old_ip_logic = r'''            is_explicit_ip_tgt = \(ip_addr in targets_set\)
            is_query_discovery = \(target_type == "discovery"\)
    ...
    new_ip_logic = '''            is_explicit_ip_tgt = (ip_addr in targets_set)
            is_query_discovery = (target_type == "discovery")
    ...
    content = re.sub(old_ip_logic, new_ip_logic, content)
    
    # 3. Only spawn IPs that are explicit targets or in discovery mode
    # And then clean up edges!
    old_visible = r'''            # Determine which IPs have actual topological connections on canvas \(either via RESOLVES_TO or CONTAINS_TARGET\)
    ...
    new_visible = '''            # Determine which IPs to spawn: only explicit targets or if in discovery mode
            # We NO LONGER spawn IPs just because a parent FQDN is marked.
            is_discovery = target_type == "discovery"
            visible_ip_nodes = [n for n in ip_nodes if is_discovery or n["data"]["ip"].lower() in targets_set]
            
            connected_ip_ids = {n["data"]["id"] for n in visible_ip_nodes}'''
            
    content = re.sub(old_visible, new_visible, content)
    
    # 4. Filter edges in build_graph to prevent Cytoscape errors from edges pointing to unspawned IPs
    old_end = r'''        return \{
            "elements": \{
                "nodes": nodes,
                "edges": edges
    ...
    new_end = '''        # Filter edges to only keep those where both source and target are spawned
